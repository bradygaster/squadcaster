...
jobs remain the authoritative observed prefix. Polling never requests workflow
logs.
...
| Workflow jobs and steps | `GET /repos/{owner}/{repo}/actions/runs/{run_id}/jobs` | Up to 20 selected runs, 10 REST pages per run, 100 jobs per page; Actions read access | Independent `workflowJobs` source and per-run cache; retain last successful jobs; distinguish fresh empty, partial, stale, unavailable, and not selected | Separate bounded drill-down; show observed fields only | Implemented in v2 |
